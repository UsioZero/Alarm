...
class AlarmsScreenState extends State<AlarmsScreen> {
  ...
  AlarmsScreenState() {
    simpleNotifications.add(SimpleNotification(
        _onAndroidSelectNotification, _onIOSSelectNotification) );
    simpleNotifications.add(SimpleNotification(
        _onAndroidSelectNotification, _onIOSSelectNotification) );
        print(simpleNotifications.length);
  }
  ...
  Future _waitTimeFuture(BuildContext context) async {
    setState(() {
       selectedTime = DateTime(_dateTime.year, _dateTime.month,
    ...
    simpleNotifications[0].notify(selectedTime);
